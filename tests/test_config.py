...
class TestLoadConfig:
    ...
    def test_returns_dict_with_expected_keys(self, tmp_path):
        ...
        env_file.write_text("MODEL_API_KEY=test-key\n", encoding="utf-8")

        config = load_config(env_path=env_file)

        assert "MODEL_API_KEY" in config
    ...
    def test_reads_values_from_env_file(self, tmp_path):
        ...
        assert config["MODEL_API_KEY"] == "sk-test123"
    ...
    def test_defaults_when_no_env_file(self, tmp_path):
        ...
        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("MODEL_API_KEY", "MODEL_BASE_URL", "DEFAULT_MODEL",
    ...
    def test_env_file_overrides_os_env(self, tmp_path):
        ...
        env_file = tmp_path / ".env"
        env_file.write_text("MODEL_API_KEY=from-file\n", encoding="utf-8")

        with patch.dict(os.environ, {"MODEL_API_KEY": "from-env"}):
            config = load_config(env_path=env_file)

        assert config["MODEL_API_KEY"] == "from-file"
