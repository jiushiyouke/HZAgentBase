...
class HookMiddleware(AgentMiddleware):
    ...
    async def abefore_agent(self, state: Any, runtime: Any) -> dict[str, Any] | None:
        """Agent 执行开始前触发 SESSION_START（异步版本）。"""
        try:
            self.executor.execute(HookEvent.SESSION_START, {
                "state_keys": list(state.keys()) if isinstance(state, dict) else [],
            })
    ...
    async def aafter_agent(self, state: Any, runtime: Any) -> dict[str, Any] | None:
        """Agent 执行结束后触发 SESSION_END（异步版本）。"""
        try:
            self.executor.execute(HookEvent.SESSION_END, {
                "state_keys": list(state.keys()) if isinstance(state, dict) else [],
            })
    ...
    async def awrap_model_call(
        ...
        """用户提交消息时触发 USER_PROMPT_SUBMIT（异步版本）。"""
        # 提取用户消息
        messages = request.messages or []
        user_content = ""
        for msg in messages:
    ...
    async def awrap_tool_call(
        ...
        """工具执行前后触发 PRE_TOOL_USE 和 POST_TOOL_USE（异步版本）。

        awrap_tool_call 接收 ToolCallRequest，包含：
        - tool_call: dict（name, args, id）
        - tool: BaseTool 实例或 None
        - state: Agent 状态
        - runtime: 运行时上下文
